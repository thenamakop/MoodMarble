...
- **Manager join-code flow:** verified via `apps/mobile/e2e/manager-join-code-journey.e2e.cjs`. A new `manager-journey.e2e.cjs` file now covers date-picker navigation, team-selector visibility, summary-panel content, and sign-out through the correct join-code auth path.
